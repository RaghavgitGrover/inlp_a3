"""
INLP Assignment 3 — Task 3: Using Language Modeling to Rectify Decryption Errors
==================================================================================
Architecture:
  Decryption Model : LSTM (Task 1) — Seq2Seq with Bahdanau Attention
  Language Model 1 : SSM / S4D (Task 2) — Autoregressive Next-Word Prediction
  Language Model 2 : Bi-LSTM (Task 2) — Masked Language Modeling

Correction Pipeline (v7 - Joint LM + Edit Distance Integration):
  1. Decode noisy cipher with LSTM decoder → character sequence + per-char confidence.
  2. Map character confidence → word confidence (average over word chars).
  3. Flag words for correction if:
       (a) The word is not in the complete plain.txt lexicon (a definitive error).
       (b) The decoder's average confidence for the word is < 0.85 (a potential error).
  4. Apply batched correction scoring candidates via: 
     Score = LM_Log_Prob - (ALPHA_ED * Edit_Distance)
"""

# ─────────────────────────────────────────────────────────────────────────────
# 0.  IMPORTS & AUTO-INSTALL
# ─────────────────────────────────────────────────────────────────────────────
...
# Paths
TASK1_DIR     = Path("/kaggle/input/datasets/raghav11grover/inlp-a3-3-final/3/task1")
TASK2_DIR     = Path("/kaggle/input/datasets/raghav11grover/inlp-a3-3-final/3/task2")
DATA_DIR      = Path("/kaggle/input/datasets/raghav11grover/inlp-a3-3-final/3/data")
OUT_RESULTS   = Path("./outputs/results")
OUT_PLOTS     = Path("./outputs/plots")

for _d in (OUT_RESULTS, OUT_PLOTS):
    _d.mkdir(parents=True, exist_ok=True)

# High-yield joint-scoring configuration
CONF_THRESHOLD   = 0.85   # Aggressive flagging
ALPHA_ED         = 2.5    # Edit distance penalty multiplier
TOP_K_PRED       = 100    # Deep candidate search for visual similarity
N_PASSES_SSM     = 3
SEQ_LEN_SSM      = 128
SEQ_LEN_BILSTM   = 128
DECODE_BATCH     = 64
...
class S4D(nn.Module):
    ...
    def forward(self, x):
        b, l, h = x.shape; dt = torch.exp(self.log_dt)
        A = -torch.exp(self.log_A_real) + 1j * self.A_imag; dtA = A * dt.unsqueeze(-1)
        B_d = (torch.exp(dtA) - 1.0) / A * self.B; t = torch.arange(l, device=x.device)
        K = torch.einsum('hn, hnl -> hl', self.C * B_d, torch.exp(dtA.unsqueeze(-1) * t.unsqueeze(0).unsqueeze(0))).real
        x_t = x.transpose(1, 2)
        K_f = torch.fft.rfft(K, n=2 * l); x_f = torch.fft.rfft(x_t, n=2 * l)
...
class GatedSSMBlock(nn.Module):
    ...
    def forward(self, x):
        x_n = self.norm1(x); v, gate = self.expand(x_n).chunk(2, dim=-1)
...
class SSMLanguageModel(nn.Module):
    def __init__(self, vocab_size, d_model, d_state, n_layers, dropout, pad_idx):
        ...
        self.head.weight = self.embed.weight; nn.init.normal_(self.embed.weight, std=0.02); nn.init.zeros_(self.head.bias)
...
class _CustomLSTM(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, bidirectional: bool = False):
        super().__init__(); self.hidden_size = hidden_size; self.bidirectional = bidirectional
        self.W_ih_f = nn.Linear(input_size, 4 * hidden_size); self.W_hh_f = nn.Linear(hidden_size, 4 * hidden_size, bias=False)
    ...
    def _lstm_dir(self, x, W_ih, W_hh, reverse=False):
        B, T, _ = x.shape; dev = x.device; gx = W_ih(x)
        h = torch.zeros(B, self.hidden_size, device=dev); c = torch.zeros(B, self.hidden_size, device=dev)
        out =[]; rng = range(T - 1, -1, -1) if reverse else range(T)
        for t in rng:
            gates = gx[:, t] + W_hh(h); i, f, g, o = gates.chunk(4, dim=-1)
...
class _MultiLayerBiLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout=0.0):
        super().__init__(); self.layers = nn.ModuleList(); self.dropouts = nn.ModuleList(); self.num_layers = num_layers
...
class BiLSTMMLM(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, n_layers, dropout, pad_idx):
        ...
        self.norm = nn.LayerNorm(2 * hidden_dim); self.proj = nn.Linear(2 * hidden_dim, embed_dim, bias=False)
        self.head = nn.Linear(embed_dim, vocab_size, bias=True)
        self.head.weight = self.embed.weight
        nn.init.normal_(self.embed.weight, std=0.02); nn.init.zeros_(self.head.bias); nn.init.orthogonal_(self.proj.weight)

    def forward(self, x): return self.head(self.proj(self.drop(self.norm(self.lstm(self.drop(self.embed(x)))))))

# ─────────────────────────────────────────────────────────────────────────────
# 4.  VOCABULARY & MODEL LOADING
# ─────────────────────────────────────────────────────────────────────────────
...
class Vocabulary:
    ...
    def load(self, path) -> "Vocabulary":
        ...
        self.word2idx = data["word2idx"]; self.idx2word = {int(k): v for k, v in data["idx2word"].items()}
        return self
...
def load_lstm_task1() -> Tuple[Seq2SeqLSTM, dict, dict, dict, dict]:
    ...
    cfg = ckpt.get("cfg", {}); E, H, L, D = (cfg.get("embed_dim", 128), cfg.get("hidden_size", 512), cfg.get("num_layers", 3), cfg.get("dropout", 0.25))
    enc = LSTMEncoder(len(t2i), E, H, L, D); dec = LSTMDecoder(len(c2i), E, H, D)
    model = Seq2SeqLSTM(enc, dec).to(DEVICE); model.load_state_dict(ckpt["model_state_dict"]); model.eval()
    ep = ckpt.get("epoch", "?"); vl = ckpt.get("val_loss", float("nan"))
...
def load_task2_models(vocab: Vocabulary) -> Tuple[SSMLanguageModel, BiLSTMMLM]:
    ssm_path = TASK2_DIR / "models" / "ssm_nwp_best.pt"; bilstm_path = TASK2_DIR / "models" / "bilstm_mlm_best.pt"
    sc = torch.load(ssm_path, map_location=DEVICE, weights_only=False); scfg = sc["config"]
    ssm = SSMLanguageModel(scfg["vocab_size"], scfg["d_model"], scfg["d_state"], scfg["n_layers"], scfg["dropout"], scfg["pad_idx"]).to(DEVICE)
    ssm.load_state_dict(sc["model_state"]); ssm.eval()
    bc = torch.load(bilstm_path, map_location=DEVICE, weights_only=False); bcfg = bc["config"]
    bilstm = BiLSTMMLM(bcfg["vocab_size"], bcfg["embed_dim"], bcfg["hidden_dim"], bcfg["n_layers"], bcfg["dropout"], bcfg["pad_idx"]).to(DEVICE)
    bilstm.load_state_dict(bc["model_state"]); bilstm.eval()
...
# ─────────────────────────────────────────────────────────────────────────────
# 5.  DATA & CORRECTION LOGIC (with Edit Distance Integration)
# ─────────────────────────────────────────────────────────────────────────────
...
def decode_cipher(lstm: Seq2SeqLSTM, plain_lines: List[str], cipher_lines: List[str], t2i: dict, c2i: dict, i2c: dict) -> Tuple[List[str], List[List[float]]]:
    ...
    for i in range(n):
        ...
        valid_idx.append(i); valid_ids.append([t2i.get(t, 1) for t in tokenize_robust(pl, cl)])
                    ...
                    for j in sorted(list(flags)):
                        ...
                        for v, pid in zip(top_vals, top_ids):
                            ...
                            if pw and pw not in SPECIAL_WORDS and pw.lower() in full_lexicon:
                                ...
                                # JOINT SCORING: LM Log Prob - (Edit Distance * Penalty Weight)
                        ...
                        if best_pred and current_words_batch[i][j] != best_pred:
                            ...
                            # Safety gate: don't alter a confident/valid word entirely
...
def process_noise_level(x: int, lstm: Seq2SeqLSTM, ssm: SSMLanguageModel, bilstm: BiLSTMMLM,
    ...
    N = min(len(plain_lines), len(cipher_lines)); pls, cls, refs = plain_lines[:N], cipher_lines[:N], plain_lines[:N]

    print(f"\n{'─'*80}\n  Noise x={x}  |  {cipher_path.name}  |  Processing {N} lines\n{'─'*80}")

    print("  [1/3] LSTM decoding ..."); t0 = time.time()
    lstm_texts, lstm_confs = decode_cipher(lstm, pls, cls, t2i, c2i, i2c)
    print(f"         done in {time.time()-t0:.1f}s")
    m_lstm = all_metrics(lstm_texts, refs); wa_lstm = m_lstm.get("word_accuracy", 0)
    print(f"         word_acc = {wa_lstm:.4f}")

    # Log LSTM-only metrics to WandB
    ...
    for text, confs in zip(lstm_texts, lstm_confs):
        words, wconfs = text_to_words_confs(text, confs); list_words.append(words); list_confs.append(wconfs)

    print("  [2/3] SSM correction ..."); t0 = time.time()
    ssm_texts =[" ".join(cw) for cw in batched_correction_engine(list_words, list_confs, ssm, vocab, full_lexicon, is_ssm=True)]
    print(f"         done in {time.time()-t0:.1f}s")
    m_ssm = all_metrics(ssm_texts, refs); wa_ssm = m_ssm.get("word_accuracy", 0); delta_s = wa_ssm - wa_lstm
    pct_s = (delta_s / wa_lstm * 100) if wa_lstm > 0 else 0
    print(f"         word_acc = {wa_ssm:.4f}  (Δ={delta_s:+.4f}, {pct_s:+.2f}%)")

    # Log LSTM+SSM metrics to WandB
...
# ─────────────────────────────────────────────────────────────────────────────
# 7.  METRICS, PLOTTING & ANALYSIS
# ─────────────────────────────────────────────────────────────────────────────
def _char_acc(p: str, r: str) -> float:
    if not r: return 1.0 if not p else 0.0
    n = min(len(p), len(r)); return sum(a == b for a, b in zip(p[:n], r[:n])) / len(r)
def _word_acc(p: str, r: str) -> float:
    pw, rw = p.split(), r.split()
    if not rw: return 1.0 if not pw else 0.0
    n = min(len(pw), len(rw)); return sum(a == b for a, b in zip(pw[:n], rw[:n])) / len(rw)
def compute_rouge(preds: List[str], refs: List[str]) -> dict:
    scorer = rs.RougeScorer(["rouge1", "rouge2", "rougeL"], use_stemmer=False); r1, r2, rl = [], [],[]
...
def compute_bleu(preds: List[str], refs: List[str]) -> dict:
    hyps =[p.split() for p in preds]; refs_w = [[r.split()] for r in refs]
...
def all_metrics(preds: List[str], refs: List[str]) -> dict:
    ...
    ps, rs_list = list(zip(*pairs)); rouge, bleu = compute_rouge(ps, rs_list), compute_bleu(ps, rs_list)
...
def make_plots(all_results: dict, noise_levels: List[int]):
    ...
    plt.tight_layout()
    plot_path = OUT_PLOTS / "task3_comparison.png"
...
def write_analysis(all_results: dict, noise_levels: List[int]):
    ...
    print("[Analysis] Written to outputs/results/task3_analysis.txt")

    # Log analysis as a WandB artifact
...
def print_full_results_table(all_results: dict, noise_levels: List[int]):
    """Print a comprehensive results table covering ALL metrics including ROUGE and BLEU."""
    METRIC_KEYS = [
...
def main():
    ...
    # ── Print full results table (all metrics including ROUGE & BLEU) ─────────
    print_full_results_table(all_results, noise_levels)

    if all_results:
        print("\n[4] Generating plots and analysis report...")
        make_plots(all_results, noise_levels)
        write_analysis(all_results, noise_levels)

    # ── Log result text files as WandB artifacts ──────────────────────────────
